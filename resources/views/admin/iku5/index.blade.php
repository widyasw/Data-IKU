                        ...
                        <div class="flex gap-2">
                            <div class="dropdown relative">
                                ...
                                <ul
                                    ...
                                    <li>
                                        <a href="{{ route('admin.iku-5.print', ['type' => 'pdf']) }}" target="_blank"
                                    ...
                                    <li>
                                        <a href="{{ route('admin.iku-5.print', ['type' => 'pdf', 'preview' => true]) }}"
                                            target="_blank"
                                            class="text-slate-600 dark:text-white block font-Inter font-normal px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600
                                                    dark:hover:text-white">
                                            Preview PDF
                                    ...
                                    <li>
                                        <a href="{{ route('admin.iku-5.print', ['type' => 'excel']) }}" target="_blank"
                                            class="text-slate-600 dark:text-white block font-Inter font-normal px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600
                                                    dark:hover:text-white">
                                            Download Excel
                                        </a>
                                    ...
                                    <tbody
                                        ...
                                        @foreach ($items as $key => $item)
                                            <tr>
                                                ...
                                                <td class="table-td">{{ $item->activity_type }}</td>
                                                <td class="table-td">{{ $item->summary }}</td>
                                                <td class="table-td">{{ $item->description }}</td>
                                                <td class="table-td">{{ $item->location }}</td>
                                                <td class="table-td">
                                                    <a href="{{ route('show_file', ['path' => 'iku-5', 'id' => $item->id, 'preview' => true]) }}"
                                                ...
                                                <td class="table-td ">
                                                    <div class="flex space-x-3 rtl:space-x-reverse">
                                                        ...
                                                        <form id="delete-form-{{ $item->id }}"
                                                            action="{{ route('admin.iku-5.destroy', $item->id) }}"
                                                            method="POST">
                                                            @method('DELETE')
                                                            @csrf
                                                            <button
                ...
                <div class="modal-dialog modal-xl relative w-auto pointer-events-none">
                    <div
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-slate-700">
                            ...
                            <!-- Modal body -->
                            <div>
                                <form id="form-el" action="{{ route('admin.iku-5.store') }}" method="POST"
                                    ...
                                    <div class="p-6 space-y-6">
                                        ...
                                        <div class="input-group">
                                            <label for="summary"
                                                class="text-sm font-Inter font-normal text-slate-900 block">Nama Hasil
                                                Kerja</label>
                                            <input type="text" id="summary" name="summary"
                                                class="text-sm font-Inter font-normal text-slate-600 block w-full py-3 px-4 border border-slate-400 rounded-md focus:outline-none focus:ring-0 mt-1"
                                                placeholder="Masukkan nama hasil kerja" required>
                                        </div>
...
@push('js')
    ...
    <script>
        ...
        function openModal() {
            reset_form();
            $('#form-title').html('Tambah IKU 5');
            $('#form_modal').modal('show');
        }

        function edit(data) {
            reset_form();
            $('#form-title').html('Edit IKU 5');
        ...
        function reset_form() {
            $("#form-el").attr("action", "{{ route('admin.iku-5.store') }}");
